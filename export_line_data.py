...
# Create a dictionary to hold the running point total for all teams for all seasons
club_sumpoints= dict()
for club in clubs:
    club_sumpoints[club[0]] = list()
...
for competition in competitions:
    ...
    for club in club_sumpoints:
        if club in ranking:
            if len(club_sumpoints[club]) == 0:
                club_sumpoints[club].append(ranking[club][1])
            else:
                club_sumpoints[club].append(club_sumpoints[club][-1] + ranking[club][1])
        else:
            if len(club_sumpoints[club]) == 0:
                club_sumpoints[club].append(0)
            else:
                club_sumpoints[club].append(club_sumpoints[club][-1])
...
for yr in range(len(seasons)):
    ...
    sumpoints_fh.write(sumpoints_string)
