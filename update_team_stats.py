...
# Update raw stats (Games Played, Win, Draw, Loss, Goals For, Goals Against)
print('\nBuilding league table from match data')
...
# Loop over all matches and update raw stats
...
"""
# Update computed stats (Goal Differental, Points)
print('\nCalculating computed league statistics')

# Select all rows in the League Table
cur.execute('''
        select comp_id, club_id, w, d, l, gf, ga
        from LeagueTable
        ''')

rows = cur.fetchall()

# Loop over all matches and update computed stats

for row in rows:
    (competition, team, w, d, l, gf, ga) = row
